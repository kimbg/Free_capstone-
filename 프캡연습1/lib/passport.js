...
module.exports = function (app) {
    ...
    //아래는 카카오 로그인
    const kakaoCredential = require('../config').kakaoCredentials;

    passport.use('kakao-login',new KakaoStrategy({        
        clientID : kakaoCredential.clientID,
        callbackURL : kakaoCredential.callbackURL
    },async(accessToken,refreshToken,profile,done)=> {
        console.log('KakaoStrategy1',kakaoCredential.clientID,);
        console.log('KakaoStrategy2',accessToken,refreshToken,profile);
        var user = {
            name : profile.displayName
        }
        done(null,user);
